  ...
  <div class="all2-div">
    ...
    <div class="contain-div">
      ...
      <div class="basicdetail-div">
        <div class="basicprice-div">参考价格  住宅<span style="font-size: 30px;font-size: 30px;vertical-align: -2px;color: #f60;margin: 0 5px;">{{detailData.ave_price}}</span> 元/㎡  </div>
        <div class="basicprice-div">楼盘户型  三室(1)，四室(1) </div>
        <div class="basicprice-div">楼盘地址  [ {{detailData.position_main}} ] {{detailData.second_main}}</div>
        <div class="blankgreen-div"><a href="#"></a><span>{{detailData.tel}}</span></div>
        <div class="basicprice-div">最新开盘   预计{{detailData.sale_time}}月开盘 </div>
        <div class="basicprice-div">交房时间  {{detailData.submit_time}}</div>
        <div class="basicprice-div">建筑类型  {{detailData.property_types}} </div>
      </div>
    </div>
...
<script>
    export default {
       ...
       data(){
          ...
          let data=this.$axios.get(`http://localhost:8080/house/${this.$route.params.id}`).then(res=>{
            let json=res.data;
            if(json.err){
              console.log('出错了')
            }else {
              dataBus.detailData=json.data;
            }
          },err=>{
           console.log('出错了',err)
          });
